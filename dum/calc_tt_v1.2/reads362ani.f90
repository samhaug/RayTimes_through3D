...
!reads362ani.f90
!
! Routine modified by Carlos Chaves from model_s362ani.f90 (SPECFEM 3D GLOBE-7.0)
!
!S362ANI
!
...
! S362ANI+M (s362anim)
!
! A global shear-wave speed model developed by Moulik and Ekstrom [2014] with normal modes
! In this model, radial anisotropy is confined to the uppermost mantle.
! The model (and the corresponding mesh) incorporate
! tomography on the 650-km and 410-km discontinuities in the 1D reference model REF.
! s362wmanim: A version of S362ANI+M with anisotropy allowed throughout the mantle.
! note: The new S362ANI model was incorporated in the specfem3d_globe7 by Carlos Chaves at the 
! University of Michigan on 05/05/2016
!--------------------------------------------------------------------------------------------------


module model_s362ani_par

  ! used for 3D Harvard models s362anim, s362wmanim
...
stat=ier)
    ...
    call read_model_s362ani(THREE_D_MODEL)
...
subroutine read_model_s362ani(THREE_D_MODEL)
    ...
    if (THREE_D_MODEL  ==  'S362ANI_M') then
        modeldef='models/S362ANI_M'
    else if (THREE_D_MODEL  ==  'S362WMANI_M') then
        modeldef='models/S362WMANI_M'
    else
...
subroutine evradker(depth,string,nker,vercof,dvercof,ierror)
    ...
    real(kind=4), parameter :: R_EARTH_KM = 6371.0
...
hsplfile,refmodel,kernstri,desckern)
    ...
    if (ios /= 0) then
        write(*,*) 'Error opening "', trim(filename), '": ', ios
    endif
...
subroutine model_s362ani_subshsv(xcolat,xlon,xrad,dvh,dvv,phase)
...
vstr(1:lstr) == 'EA SH-SV,') then
    ...
    if (phase=='S') then
        ...
        dvh=0.55*vsh3drel    ! scaling used in the inversion
        dvv=0.55*vsv3drel    ! scaling used in the inversion
    endif
